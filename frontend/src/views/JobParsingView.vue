...
async function choose(event: Event, type: 'csv' | 'file') {
  ...
  try {
    if (type === 'csv') {
      ...
      const result: any = await api.uploadData(file, 'REPORT')
      const extractedText = String(result?.extractedText || '').trim()
      if (!extractedText) throw new Error('文件已上传，但没有提取到可用于岗位解析的文字')
      jdText.value = {
        title: roleTitleFromFileName(String(result?.sourceName || file.name)),
        description: extractedText
      }
      const parsed = await parseJdText()
      if (parsed) {
        const suffix = result?.textTruncated ? '（文档较长，已解析前 30000 字）' : ''
        ElMessage.success(`行业资料已接入证据库，并完成岗位解析${suffix}`)
      }
    }
...
function roleTitleFromFileName(fileName: string) {
  return fileName
    .replace(/\.(pdf|docx?|txt|html?)$/i, '')
    .replace(/^\s*\d+\s*[_\-.、—]*\s*/, '')
    .replace(/[_]+/g, ' ')
    .replace(/\s*(岗位说明书|岗位标准|职位描述|行业资料|JD)\s*$/i, '')
    .trim()
}
...
async function parseJdText() {
  if (!jdText.value.description.trim()) return false
  ...
  try {
    ...
    return Boolean(jdParseResult.value)
  } catch (err: any) {
    ElMessage.error(err.message)
    return false
  } finally {
          ...
          <div class="intake-action-list">
            ...
            <label class="intake-action-row">
              ...
              <div>
                ...
                <small>{{ tx('上传后自动提取正文并在右侧输出岗位、技能、职责与场景。', 'Extracts the document and outputs the role, skills, responsibilities and scenarios on the right.') }}</small>
              </div>
