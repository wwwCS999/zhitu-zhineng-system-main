...
public class DataGovernanceService {
 private static final int MAX_PARSE_PREVIEW_CHARS=30_000;
 ...
 public Map<String,Object> upload(MultipartFile file,String sourceType)throws Exception{
  ...
  String name=Optional.ofNullable(file.getOriginalFilename()).orElse("upload.bin");
  String cleaned=clean(tika.parseToString(file.getInputStream()));
  if(cleaned.isBlank())throw new IllegalArgumentException("未能从文件中提取可读文字；如果是扫描版 Word/PDF，请先执行 OCR 或另存为可复制文本的 DOCX");
  Map<String,Object>result=new LinkedHashMap<>(importText(name,sourceType,null,cleaned));
  String parseText=cleaned.length()>MAX_PARSE_PREVIEW_CHARS?cleaned.substring(0,MAX_PARSE_PREVIEW_CHARS):cleaned;
  result.put("sourceName",name);
  result.put("extractedText",parseText);
  result.put("extractedCharacters",cleaned.length());
  result.put("textTruncated",cleaned.length()>MAX_PARSE_PREVIEW_CHARS);
  return result;
 }
